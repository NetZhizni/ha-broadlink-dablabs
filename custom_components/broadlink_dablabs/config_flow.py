...
from . import blk
from .blk.exceptions import AuthenticationError, BroadlinkException, NetworkTimeoutError
...
class BroadlinkFlowHandler(ConfigFlow, domain=DOMAIN):
    ...
    async def async_set_device(
        ...
        if device.type not in DEVICE_TYPES:
            _LOGGER.error(
                "Unsupported device: %s. This integration does not wire up "
                "a platform for it yet",
                hex(device.devtype),
            )
    ...
    async def async_step_finish(
        ...
        data_schema = {
            probatio.Required(CONF_NAME, default=device.model or device.name): str
        }
        return self.async_show_form(
            step_id="finish", data_schema=probatio.Schema(data_schema), errors={}
        )
